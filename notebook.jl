...
begin
	B = sqrt.(data.Bx.^2 + data.By.^2)
	h_B = heatmap(data.xvals, data.yvals, transpose(B[:,:,t]), clims=(Bmin, Bmax))
	xlabel!(h_B, "X")
	ylabel!(h_B, "Y")
	title!("B (timestep: $t)")
end
...
begin
	U = sqrt.(data.Ux.^2 + data.Uy.^2)
	h_U = heatmap(data.xvals, data.yvals, transpose(U[:,:,t]), clims=(Umin, Umax))
	xlabel!(h_U, "X")
	ylabel!(h_U, "Y")
	title!(h_U, "U (timestep: $t)")
end
...
begin 
	psi = stream_function(data.Bx, data.By);
	c_psi = contour(data.xvals, data.yvals, transpose(psi[:,:,t]), clims=(psimin, psimax), color=:black)
	xlabel!(c_psi, "X")
	ylabel!(c_psi, "Y")
